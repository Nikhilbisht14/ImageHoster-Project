...
public class ImageRepository {
    ...
    public Image getImageByTitle(Integer imageId, String title) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.id = :imageId AND i.title =:title", Image.class).setParameter("imageId", imageId).setParameter("title", title);
